...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self, cwd, parent=None):
        ...
        config_menu.addAction(QtGui.QAction('Edit Config File', self, statusTip='Edit configuration file new DAG file', triggered=self.edit_condor_config))
        config_menu.addSeparator()
        config_menu.addAction(QtGui.QAction('Reconfig local Node', self, statusTip='Command: condor_reconfig', triggered=self.reconfig))
        config_menu.addAction(QtGui.QAction('Restart local Node', self, statusTip='Command: condor_restart', triggered=self.restart))
        config_menu.addAction(QtGui.QAction('Reconfig all Nodes', self, statusTip='Command: condor_reconfig -all', triggered=self.reconfig_all))
        config_menu.addAction(QtGui.QAction('Restart all Nodes', self, statusTip='Command: condor_restart -all', triggered=self.restart_all))
        ...
        stats_menu.addAction(QtGui.QAction('Show in Browser', self, statusTip='Visit file:///C:/condor_extra/viewdir/index.html', triggered=self.statistics))
    ...
    def show_res(self):
        ...
        if self.show_mode_res == 'all':
            ...
            self.process_res.start('condor_status -autoformat:h Name OpSysName Cpus Memory Disk LoadAvg State KeyboardIdle Activity ActTime JobId')
    ...
    # Config
    #############################################################################
    def edit_condor_config(self):
        """
        Opens class EditCondorConfigWindow.
        A text editor window which can be used to edit the condor_config file.
        """
        self.editCondorConfigWindow = condor_gui_edit_config.EditCondorConfigWindow()
        self.editCondorConfigWindow.show()

    def reconfig(self):
        """
        Reconfig local master.
        """
        self.run_command('condor_reconfig')
        self.timer_res.start(5000)
        #os.system('condor_reconfig')

    def restart(self):
        """
        Restart local master.
        """
        self.run_command('condor_restart')
        self.timer_res.start(5000)
    ...
    def reconfig_all(self):
        ...
        Reconfig all nodes.
        """
        self.run_command('condor_reconfig -all')
        self.timer_res.start(5000)
    ...
    def restart_all(self):
        ...
        Restart all nodes.
        """
        self.run_command('condor_restart -all')
        self.timer_res.start(5000)
    ...
    def statistics(self):
        ...
        Show statistics.
        """
        stats_html = 'C:/condor_extra/viewdir/index.html'

        if os.path.isfile(stats_html):
            QtGui.QDesktopServices.openUrl(QtCore.QUrl('file:///' + stats_html, QtCore.QUrl.TolerantMode))
            #QtGui.QDesktopServices.openUrl('file:///C:/condor_extra/viewdir/index.html')
        else:
            QtGui.QMessageBox.warning(self, 'Statistics',
                                      'Couldn\'t find statistics file\n' + stats_html, QtGui.QMessageBox.Ok)
